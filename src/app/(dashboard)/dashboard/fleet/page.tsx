...
export default async function FleetPage() {
  ...
  return (
    <>
      <header className="px-6 py-4 border-b border-border bg-white"><h1 className="text-xl font-bold">Buses</h1><p className="text-sm text-muted">Riders see each bus as &quot;Label - {org.name}&quot;</p></header>
      <div className="p-6 grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 grid sm:grid-cols-2 gap-3">
          {(buses ?? []).length === 0 && <p className="text-muted text-sm">No buses yet.</p>}
          {(buses ?? []).map((b) => (
            <div key={b.id} className="rounded-xl border border-border bg-white p-4 flex items-center gap-3">
              ...
              <form action={deleteBus}><input type="hidden" name="id" value={b.id} /><button className="text-muted hover:text-red-500"><Trash2 className="w-4 h-4" /></button></form>
            </div>
          ))}
        </div>
        <form action={createBus} className="rounded-xl border border-border bg-white p-4 space-y-3 h-fit">
          ...
          <button className="w-full py-2 rounded-lg bg-primary hover:bg-primary-dark text-white text-sm font-medium">Add bus</button>
        </form>
      </div>
    </>
  );
